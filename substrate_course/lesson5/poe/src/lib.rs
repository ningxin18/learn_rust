...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		//转移存证
		#[weight = 0]
		pub fn transfer_claim(origin, claim: Vec<u8, reveiver: <T as frame_system::Trait>::AccountId>, ClaimTransfered) -> dispatch::DispatchResult {
			let sender = ensure_signed(origin)?;
			ensure!(Proofs::<T>::contains_key(&claim), Error::<T>::ClaimNotExist);
			let(owner, _block_number) = Proofs::<T>::get(&claim);
			ensure!(owner = sender, Error::<T>::NotClaimOwner);
			Proofs::<T>::transfer(&claim);
			Self::deposit_event(RawEvent::ClaimTransfered(sender, reveiver, claim));
			Ok(())
		}
	}
}
