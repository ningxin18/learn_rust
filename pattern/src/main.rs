...
3、在 Rust 中，可能用到模式的位置
（1）match 必须匹配完所有的情况，占位符_ 忽略掉所有情况
    match VALUE {
        PATTERN => EXPRESSION,
        PATTERN => EXPRESSION,
        PATTERN => EXPRESSION,
...
//    for循环，在for循环中，模式是直接跟随for关键字的值，
//  例如for x in y, x就是对应的模式
    let v = vec!['a', 'b', 'c'];
...
//    let, let PATTERN = EXPRESSION
    let (x, y, z) = (1, 2, 3); //(1,2,3)会匹配(x, y, z)
    println!("{},{},{}", x, y, z);
